...
## 2024-05-30 - Dynamic Button UX based on Session State
**Learning:** In Streamlit applications, dynamically updating a primary CTA button's label and disabled state (e.g., changing 'Convert' to 'Re-Convert' when settings become stale, or disabling it as 'Converted') based on session state provides excellent UX. This pattern functions correctly using Streamlit's implicit label-based button keys, provided the label evaluates consistently at the start of the rerun.
**Action:** Always use `st.rerun()` upon successful completion of the action to instantly update the UI. Ensure variables tracking state (like `st.session_state` values or locally calculated variables) are correctly populated and checked before the `st.button()` definition to avoid `NameError` or logic errors where stale settings cannot trigger a re-conversion.

## 2024-11-21 - Use Native Icons for Streamlit Alerts
**Learning:** Hardcoding emojis directly into strings used for Streamlit alerts (like `st.success`, `st.error`, `st.warning`) can cause double-emoji rendering if an icon is also provided, or create poor visual alignment compared to Streamlit's native icon layout. Furthermore, instructional messages (like warnings to re-run a process) should accurately reference the dynamic labels of the buttons they instruct the user to click.
**Action:** When displaying alerts in Streamlit apps, always remove hardcoded emojis from the text strings and pass them instead via the native `icon="🚨"` parameter. Ensure that UI copy accurately reflects the current state of dynamic elements (e.g., using 'Re-Convert' instead of 'Convert' when settings are stale).
